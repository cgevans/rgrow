        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.iter_with_setup(
                    ...
                    |(mut sys, se)| {
                        black_box(
                            sys.calc_committer(&se, 50, None, None, 5).unwrap()
                        )
                    },
                )
            },
        ...
        group.bench_with_input(
            ...
            |b, &trials| {
                b.iter_with_setup(
                    ...
                    |(mut sys, se)| {
                        black_box(
                            sys.calc_committer(&se, 50, None, None, trials).unwrap()
                        )
                    },
                )
            },
        ...
        group.bench_with_input(
            ...
            |b, &cutoff| {
                b.iter_with_setup(
                    ...
                    |(mut sys, se)| {
                        black_box(
                            sys.calc_committer(&se, cutoff, None, None, 3).unwrap()
                        )
                    },
                )
            },
...
fn bench_setup_overhead(c: &mut Criterion) {
    ...
    group.bench_function("state_creation", |b| {
        b.iter_with_setup(
            || setup_ktam_system(),
            |sys| {
                black_box(create_state_with_tiles(&sys, 3))
            },
        )
    });
...
fn create_state_with_tiles(sys: &KTAM, k: usize) -> QuadTreeState<CanvasPeriodic, NullStateTracker> {
    let mut state = sys.new_state::<QuadTreeState<CanvasPeriodic, NullStateTracker>>((32, 32)).unwrap();
