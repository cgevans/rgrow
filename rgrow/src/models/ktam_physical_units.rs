...
pub struct UKTAM {
    ...
    /// DG of links between different glues (eg, glue 1 binding to
    /// glue 2).  Should be symmetric.  Will be added with glue_strengths.
    pub glue_gibbs_at_37: Array2<Energy>,
    pub glue_entropy: Array2<EnergyPerTemperature>,
    /// kTAM $\alpha$ value (unitless, positive is favorable)
    pub second_bond_entropy: EnergyPerTemperature,
    /// Temperature, in C
    pub temperature: f64,
...
impl SystemWithDimers for UKTAM {
        ...
        for ((t1, t2), e) in self.energy_ns.indexed_iter() {
            if *e < 0. {
                ...
                dvec.push(DimerInfo {
                    ...
                    equilibrium_conc: biconc * f64::exp(-*e),
                });
            }
        }
...
impl UKTAM {
    pub fn new_sized(ntiles: Tile, nglues: Glue) -> Self {
        ...
        Self {
            ...
            glue_gibbs_at_37: Array2::zeros((nglues + 1, nglues + 1)),
            glue_entropy: Array2::zeros((nglues + 1, nglues + 1)),
            second_bond_entropy: 0.,
            temperature: 37.,
        }
    }
    ...
    pub fn update_system(&mut self) {
        ...
        for t1 in 0..(ntiles as usize) {
            for t2 in 0..(ntiles as usize) {
                ...
                self.energy_ns[(t1, t2)] = self.glue_gibbs_at_37[(t1r[2], t2r[0])]
                    - (self.temperature - 37.0) * self.glue_entropy[(t1r[2], t2r[0])];
                self.energy_we[(t1, t2)] = self.glue_gibbs_at_37[(t1r[1], t2r[3])]
                    - (self.temperature - 37.0) * self.glue_entropy[(t1r[1], t2r[3])];
            }
    ...
    pub fn monomer_detachment_rate_at_point<S: State + ?Sized>(
        ...
        self.kf * energy_exp_times_u0(self.gibbs_energy_of_tile_type_at_point(state, p, t))
    }
    ...
    fn dimer_s_detach_rate<C: State + ?Sized>(
        ...
        if (!canvas.inbounds(p2)) | (unsafe { canvas.uv_p(p2) == 0 }) | self.is_seed(PointSafe2(p2))
        {
            ...
            let t2 = unsafe { canvas.uv_p(p2) };
            {
                self.kf
                    * Rate::exp(
                        -ts - self.gibbs_energy_of_tile_type_at_point(canvas, PointSafe2(p2), t2) // FIXME
    ...
    fn chunk_detach_rate<C: State + ?Sized>(&self, canvas: &C, p: PointSafe2, t: Tile) -> Rate {
        match self.chunk_size {
            ...
            ChunkSize::Dimer => {
                let ts = { self.gibbs_energy_of_tile_type_at_point(canvas, p, t) }; // FIXME
    ...
    fn choose_chunk_detachment<C: State + ?Sized>(
        ...
        match self.chunk_size {
            ...
            ChunkSize::Dimer => {
                let ts = { self.gibbs_energy_of_tile_type_at_point(canvas, p, tile) };
...
mod tests {
    ...
    fn test_set_point_newktam<St: StateWithCreate<Params = (usize, usize)> + State>(
        ...
        let mut system = UKTAM::new_sized(5, 5);
